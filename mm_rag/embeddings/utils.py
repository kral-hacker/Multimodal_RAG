...
def get_prediction_guard_api_key():
    load_env()
    PREDICTION_GUARD_API_KEY = os.getenv("HF_API_KEY", None)
...
def get_video_id_from_url(video_url):
    ...
    if url.hostname in ('www.youtube.com', 'youtube.com'):
        ...
        if url.path[:3] == '/v/':
            return url.path.split('/')[2]
    # Return original URL if no valid ID is found (could raise an exception instead)
    return video_url
...
def get_transcript_vtt(video_url, path='/tmp'):
    ...
    # Get transcript as a list of dictionaries
    transcript = YouTubeTranscriptApi.get_transcript(video_id, languages=['en-GB', 'en'])
    
    # Convert dictionaries to objects for WebVTTFormatter
    TranscriptLine = namedtuple('TranscriptLine', ['text', 'start', 'duration'])
    transcript_objects = [TranscriptLine(**line) for line in transcript]
    
    # Format transcript to WebVTT
    formatter = WebVTTFormatter()
    webvtt_formatted = formatter.format_transcript(transcript_objects)
    
    # Write to file (no need for explicit close with 'with')
...
# Load processor & model (global initialization)
...
# def get_bridgetower_embedding(text, base64_image):
#     """
#     Generate multimodal embeddings using BridgeTower.

#     Args:
#         text (str): Caption or textual description of the image.
...
#     # Extract pooled multimodal embeddings
#     embedding = outputs.pooler_output.squeeze(0).cpu().numpy()

#     return embedding
# def bt_embedding_from_prediction_guard(prompt, base64_image):
#     # Assuming processor, model, device are defined elsewhere in the code
#     if base64_image is not None and base64_image != "":
#         if not isBase64(base64_image): 
#             raise TypeError("image input must be in base64 encoding!")
        
#         # Decode base64 image (assuming decode_base64_image is available)
#         image = decode_base64_image(base64_image)
...
#         return embedding
def bt_embedding_from_prediction_guard(prompt, base64_image):
    # Assuming processor, model, device, decode_base64_image, and isBase64 are defined elsewhere
    if base64_image and isBase64(base64_image):  # Process image-text pair
        # Decode base64 image
        image = decode_base64_image(base64_image)
        
        # Preprocess input with text and image
        inputs = processor(text=prompt, images=image, return_tensors="pt").to(device)
    ...
    # Text-only case: Prepare inputs without image
    inputs = processor.tokenizer(text=prompt, return_tensors="pt").to(device)
    with torch.no_grad():
        # Explicitly call the text model only, avoiding vision component
        outputs = model.text_model(
            input_ids=inputs["input_ids"],
            attention_mask=inputs.get("attention_mask", None),
            token_type_ids=inputs.get("token_type_ids", None)
        )
        # Use the text embeddings directly
        embedding = outputs.pooler_output.squeeze(0).cpu().numpy()
    print("Output keys (text-only):", outputs.keys())
    print("Pooler output shape (text-only):", outputs.pooler_output.shape)
    print(f"Embedding shape after squeeze (text-only): {embedding.shape}")
